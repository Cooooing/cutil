...
func SetLevel(lvl int) {
	if l, ok := DefaultLogger.(interface{ SetLevel(lvl int) }); ok {
		l.SetLevel(lvl)
	}
}

// logger implements Logger by default.
type logger struct {
	level int
}
...
func (l *logger) Debug(format string, v ...interface{}) {
	if LevelDebug >= l.level {
		_, _ = fmt.Fprintf(Output, time.Now().Format(TimeFormat)+" [DEBUG] "+format+"\n", v...)
	}
}
...
func (l *logger) Info(format string, v ...interface{}) {
	if LevelInfo >= l.level {
		_, _ = fmt.Fprintf(Output, time.Now().Format(TimeFormat)+" [INFO] "+format+"\n", v...)
	}
}
...
func (l *logger) Warn(format string, v ...interface{}) {
	if LevelWarn >= l.level {
		_, _ = fmt.Fprintf(Output, time.Now().Format(TimeFormat)+" [WARN] "+format+"\n", v...)
	}
}
...
func (l *logger) Error(format string, v ...interface{}) {
	if LevelError >= l.level {
		_, _ = fmt.Fprintf(Output, time.Now().Format(TimeFormat)+" [ERROR] "+format+"\n", v...)
	}
}
